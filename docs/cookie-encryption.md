...
WireCopy stores per-site session cookies and login credentials in the local SQLite database, encrypted with [ASP.NET Core Data Protection](https://learn.microsoft.com/aspnet/core/security/data-protection/introduction). This document covers what's encrypted, how the keys are managed, and how to operate the system safely.
...
- Linux / macOS: `~/.local/share/WireCopy/keys/key-*.xml` (file-permission protected)
- Windows: `%LOCALAPPDATA%\WireCopy\keys\` (DPAPI user-scope wrapped)
...
For sites that require authentication, WireCopy prompts for a session cookie on first navigation:

1. You log into the site in your normal browser.
2. Copy the relevant session cookie value from DevTools → Application → Cookies.
3. Paste it when WireCopy prompts.
4. WireCopy encrypts the cookie via `ICookieEncryptionService` and stores it in the SQLite `SiteCredentials` table.
...
The relevant interfaces are in `WireCopy.Application/Interfaces/`:
...
Concrete implementations live in `WireCopy.Infrastructure/Browser/` and `WireCopy.Persistence/`.
...
```bash
rm ~/.local/share/WireCopy/wirecopy.db*
```
...
- ✅ A leaked or shared `wirecopy.db` file alone — the values are unreadable without `keys/`.
...
- ❌ Malware reading process memory while WireCopy is running.
- ❌ A copy of `wirecopy.db` **and** `keys/` together.
...
- ✅ Run WireCopy as a normal user account, never elevated.
- ✅ Audit `SiteCredentials` periodically and remove stale entries.
- ❌ Don't share `keys/` or `wirecopy.db`.
