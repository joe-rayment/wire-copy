...
internal sealed class OpenAiHierarchyAnalyzer : IHierarchyAnalyzer
{
    ...
    private static string BuildInferPatternSystemPrompt(bool hasScreenshot)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("LOOK AT THE SCREENSHOT to decide story vs ad/rail — it is a PRIMARY signal,");
        sb.AppendLine("not just a lead tiebreak. Ads / sponsor posts / promos are VISUALLY OBVIOUS:");
        sb.AppendLine("they sit in labelled 'Sponsor Posts' / 'Advertisement' boxes, narrow side");
        sb.AppendLine("rails, or a separate column — EVEN WHEN their markup mimics a headline (a");
        sb.AppendLine("sponsor post styled exactly like a story). Exclude them by index. The `sect`");
        sb.AppendLine("heading corroborates: a link whose sect is 'Sponsor Posts', 'Featured");
        sb.AppendLine("Podcasts', 'Upcoming Tech Events', a 'Sponsor'/'Advertisement' label, etc. is a");
        sb.AppendLine("rail — exclude it. Column geometry corroborates too: the main story river is");
        sb.AppendLine("the WIDE, large-font, low-x column; narrow high-x side columns are rails. Use");
        sb.AppendLine("ImportanceScore/vis as corroboration, not the sole authority — a messy");
        sb.AppendLine("aggregator's DOM lies, the rendered page does not. Honour the user's answers.");
        sb.AppendLine();
