...
/// config file lives at <c>{LocalAppData}/WireCopy/bookmarks.json</c>; the
/// shipped defaults are loaded from an embedded resource in WireCopy.Persistence.
/// </summary>
public sealed class JsonBookmarkConfigStore : IBookmarkConfigStore
{
    ...
    private const string EmbeddedResourceName = "WireCopy.Persistence.Resources.bookmarks.default.json";
    ...
    public JsonBookmarkConfigStore(ILogger<JsonBookmarkConfigStore> logger)
        : this(logger, ResolveDefaultUserConfigPath(), typeof(WireCopy.Persistence.AppDbContext).Assembly)
    {
    }
    ...
    public async Task<BookmarkConfigFile> LoadShippedDefaultsAsync(CancellationToken cancellationToken = default)
    {
        await using var stream = _resourceAssembly.GetManifestResourceStream(EmbeddedResourceName)
            ?? throw new InvalidOperationException(
                ...
                "Ensure bookmarks.default.json is registered as an EmbeddedResource in WireCopy.Persistence.csproj.");
    ...
    private static string ResolveDefaultUserConfigPath()
    {
        ...
        return Path.Combine(localData, "WireCopy", "bookmarks.json");
    }
